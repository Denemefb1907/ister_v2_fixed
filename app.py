"""
İster Yönetimi v2 - ESKI WRAPPER DOSYA (KULLANIYBINMEDİ)

⚠️  BU DOSYA ARTIK BAŞLATICI DEĞİLDİR!
   Lütfen aşağıdaki komutu çalıştırın:
   
   python run.py                 # Üretim modu
   python run.py --dev           # Geliştirme modu

YENİ MVC YAPISI:
- app/                           # MVC katmanları
- app/__init__.py                 # Flask uygulama fabrikası  
- app/controllers/               # Route handler'ları
- app/models/                    # İş mantığı
- app/utils/                     # Yardımcı fonksiyonlar
- config.py                      # Konfigürasyon
- run.py                         # Uygulama başlatıcı

Detaylı bilgi: MVC_STRUCTURE.md
"""

import sys
print("❌ HATA: app.py artık başlatıcı değildir.")
print("📍 Bu komutu çalıştırın:")
print("   python run.py")
sys.exit(1)
...
def havuz_kodlarini_yeniden_sirala(platform_id, cur):
    """Havuz platformunda silme sonrası HavuzKodu (g1,g2... / b1,b2...) ve
    NodeNumarasi değerlerini mevcut SiraNo sırasına göre yeniden düzenler."""
    # Tüm isterleri SiraNo sırasıyla çek
    ...
    # --- HavuzKodu yeniden numaralandır (g1,g2... ve b1,b2...) ---
    g_say = 0; b_say = 0
    for n in nodes:
        eski_kod = n['HavuzKodu'] or ''
        if n['IsterTipi'] == 'G' and eski_kod.startswith('g'):
            g_say += 1
            yeni_kod = f"g{g_say}"
            if yeni_kod != eski_kod:
                cur.execute("UPDATE ister_node SET HavuzKodu=%s WHERE NodeID=%s",
                            (yeni_kod, n['NodeID']))
        elif n['IsterTipi'] == 'B' and eski_kod.startswith('b'):
            b_say += 1
            yeni_kod = f"b{b_say}"
            if yeni_kod != eski_kod:
                cur.execute("UPDATE ister_node SET HavuzKodu=%s WHERE NodeID=%s",
    ...
    # --- NodeNumarasi yeniden numaralandır ---
    # node_map: NodeID → node bilgisi
    node_map = {n['NodeID']: n for n in nodes}

    def yeniden_numara(parent_id, parent_num):
        """parent_id altındaki çocukları SiraNo sırasıyla 1'den başlatarak numarala."""
        cocuklar = [n for n in nodes if n['ParentID'] == parent_id]
        cocuklar.sort(key=lambda x: x['SiraNo'])
        for i, c in enumerate(cocuklar, 1):
            eski = c['NodeNumarasi'] or ''
            if parent_num:
                yeni = f"{parent_num}-{i}"
            else:
                # Kök seviye: mevcut numarayı koru (4100, 4200 vb.), sadece alt isterleri yenile
                yeni = eski
            if yeni and yeni != eski:
                cur.execute("UPDATE ister_node SET NodeNumarasi=%s WHERE NodeID=%s",
                            (yeni, c['NodeID']))
                c['NodeNumarasi'] = yeni  # sonraki recursion için güncelle
            # Alt isterlerine de uygula
            yeniden_numara(c['NodeID'], yeni)

    # Kök node'ların alt isterlerini yenile
    kok_nodlar = [n for n in nodes if not n['ParentID']]
    for kok in kok_nodlar:
        yeniden_numara(kok['NodeID'], kok['NodeNumarasi'] or '')
...
def ister_node_sil(nid):
    ...
    # Havuz platformunda silme sonrası kodları yeniden sırala
    if havuz_mu:
        havuz_kodlarini_yeniden_sirala(platform_id, cur)
    mysql.connection.commit(); cur.close(); return jsonify({'ok': True})
...
def rapor_karsilastirma():
    ...
    if platformlar:
        ...
        cur.execute(f"""SELECT n.PlatformID, n.HavuzKodu, n.NodeNumarasi, n.Icerik, n.DegistirildiMi
                       FROM ister_node n
        ...
        plat_isterler = []
    cur.close()
    # plat_map: {havuz_kodu: {str(platform_id): {NodeNumarasi, Icerik, DegistirildiMi}}}
